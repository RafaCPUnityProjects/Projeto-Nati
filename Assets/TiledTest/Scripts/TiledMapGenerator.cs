...
{
    public int xmax = 80, ymax = 25, inobjs = 0;

    void Start()
    {
        FileManager.SetupFileManager("Test00");
        Dungeon dungeon = new Dungeon(new System.Random("0".GetHashCode()), PrintOutput);
        dungeon.CreateDungeon(xmax, ymax, inobjs);
        dungeon.ShowDungeon();
...
public class Dungeon
{
    ...
    public Dungeon(System.Random rnd, Action<string> logger)
    {
        _rnd = rnd;
        _logger = logger;
    }
    ...
    public class PointDirectionTile //: IEnumerable
    {
        public PointI pointI;
        public Direction direction;
        public Tile tile;

        public PointDirectionTile(PointI pointI, Direction direction)
        {
            this.pointI = pointI;
        ...
        public PointDirectionTile(PointI pointI, Direction direction, Tile tile) : this(pointI, direction)
        {
            this.tile = tile;
        }
    }

    public IEnumerable<PointDirectionTile> GetSurroundingPoints(PointI v)
    {
        var points = new[]
                         {
                                 new PointDirectionTile(new PointI { X = v.X, Y = v.Y + 1 }, Direction.North),
                                 new PointDirectionTile(new PointI { X = v.X - 1, Y = v.Y }, Direction.East),
                                 new PointDirectionTile(new PointI { X = v.X , Y = v.Y-1 }, Direction.South),
                                 new PointDirectionTile(new PointI { X = v.X +1, Y = v.Y  }, Direction.West),

                             };
        return points.Where(p => InBounds(p.pointI));
    }

    public IEnumerable<PointDirectionTile> GetSurroundings(PointI v)
    {
        return
            this.GetSurroundingPoints(v)
                .Select(r => new PointDirectionTile(r.pointI, r.direction, this.GetCellType(r.pointI.X, r.pointI.Y)));
    }
    ...
    public bool MakeRoom(int x, int y, int xlength, int ylength, Direction direction)
    {
        ...
        if (
            points.Any(
                s =>
                s.Y < 0 || s.Y > this._ysize || s.X < 0 || s.X > this._xsize || this.GetCellType(s.X, s.Y) != Tile.Unused))
            return false;
        _logger(
                  string.Format(
    ...
    public void ShowDungeon()
    {
        ...
        for (int y = 0; y < this._ysize; y++)
        {
            ...
            {
                output += (GetCellTile(x, y).ToString());
        ...
        for (int countingTries = 0; countingTries < 1000; countingTries++)
        {
            ...
            for (int testing = 0; testing < 1000; testing++)
            {
                ...
                if (GetCellType(newx, newy) == Tile.DirtWall || GetCellType(newx, newy) == Tile.Corridor)
                {
                    ...
                    var canReach =
                        surroundings.FirstOrDefault(s => s.tile == Tile.Corridor || s.tile == Tile.DirtFloor);
                    if (canReach == null)
                    {
                        continue;
                    }
...
public static class Tuple
{
    ...
    internal static void Create(Dungeon.PointI point, Dungeon.Direction direction)
    {
